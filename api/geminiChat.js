...
async function sendDM(recipientId, text) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    console.log("[sendDM] chunk", i, "status:", res.status, "response:", JSON.stringify(data));
    results.push(data);
  }
  return results;
}
  ...
  if (req.method === "POST") {
    try {
      ...
      if (body.object !== "instagram") {
        console.log("[webhook] not instagram, ignoring");
      ...
      for (const entry of body.entry || []) {
        for (const event of entry.messaging || []) {
          ...
          if (event.message?.is_echo) {
            console.log("[webhook] echo, skipping");
            continue;
          }

          const igUserId = event.sender.id;
          const messageText = event.message?.text;
          console.log("[webhook] sender:", igUserId, "| message:", messageText);
          ...
          // Process BEFORE sending 200 so Vercel doesn't kill the function
...
async function processMessage(igUserId, userMessage) {
  console.log("[process] start for user:", igUserId);

  const [profile, chatHistory] = await Promise.all([
    getProfile(igUserId),
    getChatHistory(igUserId),
  ]);
  console.log("[process] profile loaded:", JSON.stringify(profile));
  ...
  if (updates) {
    console.log("[process] profile updates extracted:", JSON.stringify(updates));
  ...
  console.log("[process] Gemini response length:", text?.length);
  console.log("[process] Gemini response:", text);
  ...
  const sendResult = await sendDM(igUserId, text);
  console.log("[process] DM sent:", JSON.stringify(sendResult));
}
